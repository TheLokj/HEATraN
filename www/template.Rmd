...
if ("Whole Data Inspection (VolcanoPlot + Table)" %in% params$export_choices) {
...
if (!is.null(params$goGseaResults) && isTRUE(params$includeGSEAPlots)) {
  library(grid)
...
# 1. Voies ORA
if ("Pathway ORA" %in% params$export_choices) {
  if (!is.null(params$pathwayORAEnrichment) & params$includePathwayViews) {
    cat("## Static visualisation – (ORA)\n\n")
    ora_res <- params$pathwayORAEnrichment$enrichment@result
    ora_imgs <- params$pathwayViewPlots$ORA   # voir note ci-dessous
    for (i in seq_len(nrow(ora_res))) {
      id   <- ora_res$ID[i]
      desc <- ora_res$Description[i]
      cat("### ORA Pathway:", id, "–", desc, "\n\n")
      if (!is.null(ora_imgs) && length(ora_imgs) >= i && !is.null(ora_imgs[[i]])) {
        knitr::include_graphics(ora_imgs[[i]])
        cat("\n\n")
      }
    }
  }
...
if ("Pathway GSEA" %in% params$export_choices) {
  if (!is.null(params$pathwayGSEAEnrichment)) {
    if (isTRUE(params$includeGSEAPlots) || isTRUE(params$includePathwayViews)) {
    cat("## Static visualisation – (GSEA)\n\n")
    gsea_res <- params$pathwayGSEAEnrichment$enrichment@result
    gsea_imgs <- params$pathwayViewPlots$GSEA 
    for (i in seq_len(nrow(gsea_res))) {
      ...
      if (!is.null(gsea_imgs) && length(gsea_imgs) >= i && !is.null(gsea_imgs[[i]]) &&   (isTRUE(params$includePathwayViews))) {
        if (!is.null(gsea_imgs[[i]])) knitr::include_graphics(gsea_imgs[[i]])
        cat("\n\n")
      }}
    }
  }
